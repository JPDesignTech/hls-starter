...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('Corruption check request:', {
      filename,
      originalName,
      gcsPath,
    });
    ...
    if (gcsPath) {
      try {
        ...
        if (!bucketName) {
          throw new Error(
            'GCS bucket name not configured. Please set GCS_BUCKET_NAME environment variable.'
          );
        }

        console.log(
          `Attempting to download from GCS: bucket=${bucketName}, path=${gcsPath}`
        );
        ...
        if (!exists) {
          ...
          throw new Error(
            `File not found in GCS: ${gcsPath}. The file may still be uploading or the path is incorrect.`
          );
        }
        ...
        filePath = path.join(
          tempDir,
          `corruption-check-${Date.now()}-${path.basename(gcsPath)}`
        );
        ...
        return NextResponse.json(
          { error: `Failed to download file from storage: ${errorMessage}` },
          { status: 500 }
        );
      }
    } else {
    ...
    if (!ffprobeServiceUrl) {
      ...
      return NextResponse.json(
        { error: 'FFprobe service not configured' },
        { status: 500 }
      );
    }
    ...
    if (gcsPath) {
      ...
      if (!ffprobeResponse.ok) {
        ...
        throw new Error(
          `FFprobe analysis failed: ${ffprobeResponse.status} - ${errorText}`
        );
      }
      ...
      console.log(
        'FFprobe result:',
        JSON.stringify(ffprobeResult).substring(0, 200) + '...'
      );
      ...
      if (!errorOutput) {
        ...
        try {
          const errorDetectionResponse = await fetch(
            `${ffprobeServiceUrl}/probe`,
            {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
          ...
          console.log(
            'Error detection scan failed, continuing with basic analysis'
          );
        }
      }
    } else {
      ...
      ffprobeData = (await ffprobeResponse.json()) as FfprobeData;
    }

    // Analyze the results for corruption issues
    const result = analyzeForCorruption(
      ffprobeData,
      errorOutput,
      { size: stats.size },
...
function analyzeForCorruption(
  ...
  if (videoStream) {
    ...
    metadata.resolution = `${videoStream.width ?? 0}x${videoStream.height ?? 0}`;
  ...
  if (containerFormat === 'mp4' || containerFormat === 'mov') {
    if (
      errorOutput.includes('moov atom not found') ||
      ...
      issues.push({
        ...
        severity: 'critical',
        description:
          'The video file is missing essential metadata (moov atom) required for playback',
        detection: 'moov atom not found',
        fixCommand:
          'ffmpeg -i input.mp4 -c copy -movflags +faststart output.mp4',
        explanation:
          'This typically happens with incomplete downloads or improperly finalized recordings. The fix attempts to rebuild the container structure.',
      });
    }
  }
  ...
  if (
    errorOutput.includes('Could not find codec parameters') ||
    errorOutput.includes('unspecified size') ||
    errorOutput.includes('unknown codec')
  ) {
    const streamMatch = /stream\s+(\d+)/i.exec(errorOutput);
    ...
    issues.push({
      ...
      fixCommand:
        'ffmpeg -probesize 100M -analyzeduration 100M -i input.mkv -c copy output.mkv',
      explanation:
        'Codec parameters might be located later in the file. Increasing probe size can help FFmpeg find them.',
    });
  }

  // 3. Check for non-monotonic timestamps - More specific pattern
  if (
    errorOutput.includes('Non-monotonous DTS') ||
    ...
    issues.push({
      ...
      severity: 'warning',
      description:
        'The video has out-of-order timestamps which can cause playback issues',
      detection: 'Non-monotonous DTS detected',
      fixCommand: 'ffmpeg -fflags +genpts -i input.mp4 -c copy output.mp4',
      explanation:
        'Timestamps are not in the correct order. This often happens after improper editing or concatenation.',
    });
  }
  ...
  if (videoStream && audioStream) {
    ...
    const videoDuration = videoStream.duration
      ...
      : videoStream.duration_ts && videoStream.time_base
        ? eval(videoStream.time_base) * parseFloat(String(videoStream.duration_ts))
        : 0;
    const audioDuration = audioStream.duration
      ...
      : audioStream.duration_ts && audioStream.time_base
        ? eval(audioStream.time_base) * parseFloat(String(audioStream.duration_ts))
        : 0;

    if (videoDuration > 0 && audioDuration > 0) {
      const drift = Math.abs(videoDuration - audioDuration);
      if (drift > 0.5) {
        // More than 0.5 second drift
        issues.push({
          ...
          fixCommand:
            'ffmpeg -i input.mp4 -c:v copy -af "aresample=async=1:first_pts=0" -c:a aac output.mp4',
          explanation:
            'The audio will gradually go out of sync. The fix resamples audio to match video timing.',
        });
      }
    }
    ...
    if (startDiff > 0.1) {
      // More than 100ms start difference
      issues.push({
        ...
        explanation:
          'Different start times can cause constant A/V sync offset. The fix aligns the streams.',
      });
    }
  }
  ...
  if (foundDecodeErrors.length > 0) {
    issues.push({
      ...
      severity: 'warning',
      description:
        'Some video frames are corrupted and may cause visual artifacts',
      detection: foundDecodeErrors.join(', '),
      fixCommand:
        'ffmpeg -err_detect ignore_err -i input.mp4 -c:v libx264 -c:a aac output.mp4',
      explanation:
        'Re-encoding the video will skip or interpolate damaged frames, though some quality loss may occur.',
    });
  }
  ...
  if (!videoStream && !audioStream) {
    issues.push({
      ...
      explanation:
        'The file may be severely corrupted or not a valid media file.',
    });
  }
  ...
  if (videoStream && videoStream.avg_frame_rate !== videoStream.r_frame_rate) {
    issues.push({
      ...
      severity: 'info',
      description:
        'The video has variable frame rate which may cause issues in some editors',
      ...
      explanation:
        'Some applications require constant frame rate. This converts to 30fps constant.',
    });
  }
  ...
  if (
    ...
    !supportedVideoCodecs.includes(videoStream.codec_name.toLowerCase())
  ) {
    issues.push({
  ...
  if (
    ...
    !supportedAudioCodecs.includes(audioStream.codec_name.toLowerCase())
  ) {
    issues.push({
  ...
  if (audioStream) {
    ...
    if (
      ...
      issues.push({
        ...
        explanation:
          'Channel layout mismatch can cause phase issues or silent channels.',
      });
    }
  }
  ...
  if (containerFormat === 'webm') {
    issues.push({
      ...
      severity: 'info',
      description:
        'WebM format may have compatibility issues with some editing software',
      detection: `Container: ${containerFormat}`,
      fixCommand:
        'ffmpeg -i input.webm -c:v libx264 -c:a aac -movflags +faststart output.mp4',
      explanation:
        'Converting to MP4 with H.264/AAC ensures maximum compatibility with editing software like Descript.',
    });
    ...
    if (
      ...
      issues.push({
        ...
        severity: 'warning',
        description:
          'WebM files often use variable frame rate which can cause sync issues',
        detection: 'WebM with VFR',
        fixCommand:
          'ffmpeg -i input.webm -vf "fps=30" -c:v libx264 -c:a aac -af "aresample=async=1:first_pts=0" output.mp4',
        explanation:
          'This converts to constant 30fps MP4 with audio resampling to maintain sync.',
      });
    }
  }
  ...
  if (
    metadata.fps &&
    ((metadata.fps as number) < 10 || (metadata.fps as number) > 120)
  ) {
    issues.push({
      ...
      explanation:
        'Standard frame rates (24, 30, 60 fps) are more widely compatible.',
    });
  }
  ...
  issues.forEach((issue) => {
    if (issue.fixCommand && containerFormat !== 'Unknown') {
      ...
      issue.fixCommand = issue.fixCommand.replace(
        /input\.(mp4|mkv|avi|webm|mov)/g,
        `input.${containerFormat}`
      );

      // For output, default to MP4 unless fixing a specific container issue
      if (
        !issue.fixCommand.includes('output.') ||
        issue.fixCommand.includes('output.mp4')
      ) {
